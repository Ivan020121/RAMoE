...
class PRISM(nn.Module):
    ...
    def forward(self, imu_input: torch.Tensor, kp_input: torch.Tensor):
        ...
        mid_s = self.classifier1(torch.concat([modal_out, moe_output], dim=1))
...
def pca2D(model, test_loader, device, pic_tag=""):
    ...
    ax.set_xlabel(f"PC1 ({pca_obj.explained_variance_ratio_[0]:.2%})", fontsize=12)
    ax.set_ylabel(f"PC2 ({pca_obj.explained_variance_ratio_[1]:.2%})", fontsize=12)
    ax.set_title(f"PCA {pic_tag}", fontsize=14, fontweight="bold")
...
if __name__ == "__main__":
    ...
    pca2D(model, test_loader, device, pic_tag="Before Training1")

    model.load_state_dict(
        torch.load(checkpoint_path / "PRISM 2026-02-01 22:21:27-latest.pt"), strict=False
    )
    pca2D(model, test_loader, device, pic_tag="After Training")
