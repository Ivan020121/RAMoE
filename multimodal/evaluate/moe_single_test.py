...
class MoEAttn(nn.Module):
    ...
    def forward(self, imu_input: torch.Tensor, kp_input: torch.Tensor):
        ...
        expert_outputs = torch.stack(
            [out_imu, out_kp, out_imukp, out_kpimu], dim=1
        )  # (B, 4, 512)
        raw_weights = [0, 0, 0, 0]
        raw_weights[self.enable_export] = 1
        router_weights = torch.tensor(raw_weights, device=imu_features.device)  # (4,)
        # 加权融合
...
if __name__ == "__main__":
    ...
    model = MoEAttn(imu_encoder, kp_encoder, 512, dropout=0.3)
    model.load_state_dict(
        torch.load(
            checkpoint_path / "MOE 2026-01-25 16:40:00-latest.pt",
            map_location=device,
        )
    )
    expert_output = []
    for i in range(allowed_expert):
        model.enable_export = i

        expert_output.append(
            class_output(model, test_loader, device=device, modal="multimodal")
        )
    expert_rst = []
    for i in range(0, allowed_expert):
        expertA_output = expert_output[i]
        for j in range(i + 1, allowed_expert):
            print(f"Expert {i+1} VS Expert {j+1}:")
            expertB_output = expert_output[j]
            statistics, pvals, qvals = paried_test(expertA_output, expertB_output)
            for c in range(10):
                print(
                    f"Class {c}: statistic={statistics[c]:>8.2f}, p-value={pvals[c]:.4e}, q-value={qvals[c]:.4e}"
                )
