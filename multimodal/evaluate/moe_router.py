...
if __name__ == "__main__":
    ...
    # both
    _, router = class_output(
        model,
        test_loader,
        device=device,
        modal="multimodal",
        with_moe_weights=True,
    )

    for c in range(10):
        class_router = router[c]
        mean_weights = class_router.mean(axis=0)
        print(
            f"Class {c} Mean: "
            + ", ".join(f"{mean_weights[i]:.4f}" for i in range(mean_weights.size))
        )
    # only imu
    _, router = class_output(
        model,
