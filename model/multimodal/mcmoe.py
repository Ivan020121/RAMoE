...
class MoEAttn(nn.Module):
    def __init__(
        ...
        self.router_mix = Mlp(fusion_dim, fusion_dim // 2, 4, drop=dropout)
        self.top_k = topk
    ...
    def forward(self, imu_input: torch.Tensor, kp_input: torch.Tensor):
        ...
        router_logits = torch.stack(
            [
                self.router_mix(imu_proj),  # (B, 10)
                self.router_mix(kp_proj),
                self.router_mix(imu_enhance_kp),
                self.router_mix(kp_enhance_imu),
            ],
            dim=1,
        )  # (B, 4)

        top_k_logits, indices = router_logits.topk(self.top_k, dim=-1)
        zeros = torch.full_like(router_logits, float("-inf"))
        sparse_logits = zeros.scatter(-1, indices, top_k_logits)
        router_weights = F.softmax(sparse_logits, dim=-1)
        ...
        # 专家网络处理
        imu_fused = self.expert_fused(
            imu_raw_t, router_weights[:, 0, :]
        )  # (B, fusion_dim*4)
        kp_fused = self.expert_fused(
            kp_raw_t, router_weights[:, 1, :]
        )  # (B, fusion_dim*4)
        imukp_fused = self.expert_fused(
            kp_attn_t, router_weights[:, 2, :]
        )  # (B, fusion_dim*4)
        kpimu_fused = self.expert_fused(
            imu_attn_t, router_weights[:, 3, :]
        )  # (B, fusion_dim*4)

        # 加权融合
        moe_output = torch.cat([imu_fused, kp_fused, imukp_fused, kpimu_fused], dim=1)

        s = self.classifier(moe_output)  #  (B, 10)
        return s

    def expert_fused(self, feat, router_weights):
        return (
            router_weights.unsqueeze(-1)
            * torch.stack(
                [
                    self.experts["imu"](feat).squeeze(-1),
                    self.experts["kp"](feat).squeeze(-1),
                    self.experts["imukp"](feat).squeeze(-1),
                    self.experts["kpimu"](feat).squeeze(-1),
                ],
                dim=1,
            )
        ).mean(
